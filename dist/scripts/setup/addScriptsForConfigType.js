...
import { addScripts, getConflictingScripts, isValidPackageJson, readPackageJson, writePackageJson, } from '../utils/packageJsonUtils.js';
/**
 ...
 */
export const addScriptsForConfigType = async (configType) => {
    ...
    try {
        ...
        if (!existsSync(packageJsonPath)) {
            console.error(`${CLI_PROGRESS_ITEM_INDENT}${Icons.ERROR} No package.json found in directory`);
            return;
        }
        // Read existing package.json
        const existingPackageJson = readPackageJson(directory);
        if (!existingPackageJson || !isValidPackageJson(existingPackageJson)) {
            console.error(`${CLI_PROGRESS_ITEM_INDENT}${Icons.ERROR} Invalid or corrupted package.json`);
            return;
        }
        // Get scripts to add
        ...
        const filteredScriptsToAdd = scriptsToAdd.filter((script) => !scriptsToSkip.includes(script.name));
        ...
        const scriptsRecord = Object.fromEntries(filteredScriptsToAdd.map((script) => [script.name, script.command]));
        ...
        filteredScriptsToAdd.forEach((script) => {
            console.log(`${CLI_PROGRESS_ITEM_INDENT}${Icons.SUCCESS} Added '${script.name}' package.json script`);
        });
        // Log skipped scripts
        if (scriptsToSkip.length > 0) {
            scriptsToSkip.forEach((scriptName) => {
        ...
        console.error(`${CLI_PROGRESS_ITEM_INDENT}${Icons.ERROR} Failed to add package.json scripts: ${errorMessage}`);
    }
};
//# sourceMappingURL=addScriptsForConfigType.js.map