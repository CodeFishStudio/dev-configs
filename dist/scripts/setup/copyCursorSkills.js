 ...
 */
export const copyCursorSkills = (selectedSkillIds) => {
    ...
    try {
        ...
        if (totalSkipped > 0) {
            logStep(`Skipped copying ${totalSkipped} agent skill file${totalSkipped === 1 ? '' : 's'}`, 'skipped');
        }
        if (totalCopied > 0) {
            logStep(`Copied ${totalCopied} agent skill file${totalCopied === 1 ? '' : 's'}`, 'success');
        }
    }
    catch (error) {
