 ...
 */
export async function promptMultipleChoice(title, options) {
    let selectedIndex = 0;
    const selectedOptions = new Set(options.map((option) => option.value)); // Start with all selected
    const renderMenu = () => {
        // Clear screen and move cursor to top
        stdout.write('\x1B[2J\x1B[0f');
        console.log(`${TextStyles.CYAN}?${TextStyles.RESET}${TextStyles.BOLD} ${title}:${TextStyles.RESET}${TextStyles.GRAY} › Use arrow-keys to navigate, space to toggle, return to submit.${TextStyles.RESET}`);
...
//# sourceMappingURL=promptMultipleChoice.js.map