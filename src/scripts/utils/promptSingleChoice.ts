 ...
 */
export async function promptSingleChoice<T extends string>(
    title: string,
    ...
    const renderMenu = () => {
        ...
        console.log(
            `${TextStyles.CYAN}?${TextStyles.RESET}${TextStyles.BOLD} ${title}:${TextStyles.RESET}${TextStyles.GRAY} › Use arrow-keys. Return to submit.${TextStyles.RESET}`
        );
        ...
        options.forEach((option, index) => {
            ...
            if (isSelected) {
                ...
                console.log(
                    `${TextStyles.CYAN}❯ ${CLI_OPTION_INDENT}${TextStyles.UNDERLINE}${option.label}${TextStyles.RESET}`
                );
            } else {
        ...
        const handleKeyPress = (key: string) => {
            switch (key) {
                ...
                case KeyCodes.ENTER: {
                    ...
                    if (selectedOption) {
                        resolve(selectedOption.value as T);
                    } else {
